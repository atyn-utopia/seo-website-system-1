      ...
      <section className="hero-sec" style={{ background: 'var(--paper-1)' }}>
        <div className="hero-grid">
            <div className="hero-copy">
              ...
              <img src="/images/brand/logo-dark.png" alt={tHomeHeroLogo('logoAlt')} className="hero-logo" />

              <h1 className="hero-h1">{tHero('headline', { city })} {tHero('headlineHighlight', { city })}</h1>
              <h2 className="hero-sub">{tHero('subheadline', { city })}</h2>

              <div className="hero-rail">
                <span className="hero-lbl">{tHomeProducts('fromLabel')}</span>
                <b>{tHomeProducts('priceFromSqft', { price: '3.50' }).replace(/^(?:Dari|From)\s+/i, '').replace(/\s*起$/, '')}</b>
                <em>{tHomeHeroLogo('priceNote')}</em>
              </div>
