 ...
 *
 * Styling lives in globals.css, not a styled-jsx block: this element is passed
 * into the client SiteHeader as a prop, so scoped styles there would never
 * reach it.
 */
export default async function ContactNumber({
  ...
  className?: string;
  /**
   * Locale-stripped path of the page this renders on, e.g. `/`,
   * `/electric-wheelchair/kuala-lumpur`. Required for correctness, not
   * decoration: `is_display` is unique per (website, page_slug), so a site with
   * per-page display numbers needs the page to pick the right one.
   */
  page?: string;
}) {
